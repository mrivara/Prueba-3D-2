...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        //agregado modo combate 
        [Space(10)]
        [Tooltip("Habilita modo combate")]
        ...
        private bool moveHeavyAttack;
        [SerializeField] private bool EvadeLeft;
        [SerializeField] private bool EvadeRight;
        ...
        private void Start()
        {
            ...
            //Hit metodo video  https://www.youtube.com/watch?v=c0AYpO61AmE
            CountAttackClick = 0;
            moveActive = true;
            attack = false;
            KatanaController();
        ...
        private void Update()
        {
            ...
            if (_input.Hit && Grounded && Combat)
            {
                //attack = true;
                CountAttackClick++;
                Debug.Log("Cantidad de click: " + CountAttackClick);
                _input.Hit = false;
            ...
            //Hit right click
            HitHeavyStart();
            //disable hit input not combat mode
            if (_input.Hit && !Combat)
            {
                _input.Hit = false;
                CountAttackClick = 0;
            }
            ...
            if (moveAttack)
            {
                //move frame with player attack 
                _controller.Move(transform.forward * (1.5f * Time.deltaTime));
            }
            if (moveHeavyAttack)
            {
                //move frame with player attack 
                _controller.Move(transform.forward * (4f * Time.deltaTime));
            }
            if (EvadeLeft)
            {
                _controller.Move(-transform.right * (5f * Time.deltaTime));
            }
            if (EvadeRight)
            {
                _controller.Move(transform.right * (5f * Time.deltaTime));
        ...
        private void AssignAnimationIDs()
        {
            ...
            _animIDHitHeavy = Animator.StringToHash("HitHeavy");
        ...
        private void HitHeavyStart()
        {
            if (Combat && _input.HitHeavy && Grounded)
            {
                moveActive = false;
                _animator.SetBool("HitHeavy", true);
            }
            else if (!_input.HitHeavy)
            {
                moveActive = true;
                _animator.SetBool("HitHeavy", false);
            }
        ...
        public void Evade()
        {
            if (_input.EvadeLeft && Combat && Grounded)
            {
                _animator.SetInteger("Evade", 1);               
                _input.EvadeLeft = false;               
            }
            else if (_input.EvadeRight && Combat && Grounded)
            {
                _animator.SetInteger("Evade", 2);
                _input.EvadeRight = false;
            }
            else
            {
                _input.EvadeLeft = false;
...
#region Listener
        ...
        public void CheckAttackPhase()
        {
            ...
            if (_animator.GetCurrentAnimatorStateInfo(0).IsName("Hit1"))
            {
                Debug.Log("Current State: Hit1");
                if (CountAttackClick >1)
                {
                    //next attack phase: 2
            ...
            else if (_animator.GetCurrentAnimatorStateInfo(0).IsName("Hit2"))
            {
                Debug.Log("Current State: Hit2");
            ...
            {
                Debug.Log("Current State: Hit3");
                if (CountAttackClick > 3)
                {
                    //next attack phase: 2
                    _animator.SetInteger("intAttackPhase", 4);
                }
                else
                {
                    //reset attack phase
            ...
            else if (_animator.GetCurrentAnimatorStateInfo(0).IsName("Hit4"))
            {
                Debug.Log("Current State: Hit4");
        ...
        {
            moveHeavyAttack = true;
        }

        public void HeavyAttackStop()
        {
            moveHeavyAttack = false;
        }

        public void MoveEvadeLeft()
        {
            EvadeLeft = true;
            Debug.Log("evade = " + EvadeLeft);
        }
        public void MoveEvadeLeftOff()
        {
            EvadeLeft = false;
            Debug.Log("evade = " + EvadeLeft);
        }
        public void MoveEvadeExit()
        {
            _animator.SetInteger("Evade", 0);
        }
        public void MoveEvadeRight()
        {
            EvadeRight = true;
            Debug.Log("evade = " + EvadeLeft);
        }
        public void MoveEvadeRightOff()
        {
            EvadeRight = false;
            Debug.Log("evade = " + EvadeLeft);
        }
